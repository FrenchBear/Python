# pa_divers.py - Podcasts Archives (Radio France), Tout l'humour d'Inter
#
        ...
        for ma in find_iter:
            ...
            if graph.get("@type")=="RadioEpisode":
                ...
                try:
                    ...
                    filename = "C:\\downloads\\podcasts-archives-l-humour-d-inter\\" + serie + "\\" + sanitize_filename(f"{date_created:%Y-%m-%d} - {title}.{ext}")
...
for p in range(22, 51):
    print("--------------------------------------")
    print(f"Page {p}\n")
    process_page(f"https://www.radiofrance.fr/franceinter/podcasts/bouquet-l-humour-d-inter?p={p}")
