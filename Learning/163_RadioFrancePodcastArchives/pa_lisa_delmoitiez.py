        ...
        for ma in find_iter:
            ...
            if graph.get("@type")=="RadioEpisode":
                ...
                try:
                    ...
                    filename = "C:\\downloads\\podcasts-archives-lisa-delmoitiez\\" + serie + "\\" + sanitize_filename(f"{date_created:%Y-%m-%d} - {title}.{ext}")
...
for p in range(1, 25):
    print("--------------------------------------")
    print(f"Page {p}\n")
    process_page(f"https://www.radiofrance.fr/franceinter/podcasts/la-chronique-de-lisa-delmoitiez?p={p}")
