...
from common import normalize_serie, get_safe_name
    ...
    for fullpath in files:
        ...
        if to_rename:
            ...
            if DO_IT:
                try:
                    os.rename(
                        fullpath, get_safe_name(os.path.join(path, newname), fullpath)
                    )
                except:
                    print("*** Err")
    if not DO_IT:
            ...
            if folder != dicspo[folder_lna]:
                ...
                if DO_IT:
                    ...
                    try:
                        os.rename(
                            folderfp,
                            get_safe_name(os.path.join(source, newname), folderfp),
                        )
                    except:
                        print("*** Err")
    if not DO_IT:
