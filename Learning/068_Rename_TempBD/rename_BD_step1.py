...
from common import clean_file_name, normalize_serie, get_safe_name, transtab
...
def Step1(out: TextIO):
    ...
    for fullpath in get_all_files(source):
        ...
        newname = re.sub(
            r"(BD|Comics|Manga)[ \.-]*FR", " ", newname, flags=re.IGNORECASE
        )
        newname = re.sub(
            r"Pressecitron|iBooker|FRENCH|\bHYBRiD\b|eBook|WEBRip|BDPACK|STC|REPACK|RESCAN|BROKER|One shot|MagPF|Full Color|Adult Comic|Bdstudio|by 5Cobres|By Le mulot|caso DKFR|DKFR",
            "",
            newname,
            flags=re.IGNORECASE,
        )
        newname = re.sub(
            r" OS | BDX FR | BDX | FR | HD | HQ | Pack ",
            " ",
            newname,
            flags=re.IGNORECASE,
        )
        ...
        if ma := re.fullmatch(
            r"(.*), (\d\d) FRN( |,)+", newname, re.IGNORECASE
        ):  # "Agharta, 05 FRN , ,"
            newname = ma.group(1) + " - " + ma.group(2)
        if ma := re.fullmatch(
            r"(.*), (\d\d) (.+), FRN( |,)+", newname, re.IGNORECASE
        ):  # "Asphodele, 03 L'ange noir, FRN , ,"
            ...
            if DO_IT:
                try:
                    os.rename(
                        ...
                        get_safe_name(os.path.join(path, newname + ext.lower())),
                    )
                except:
                    out.write("*** Err\n")
    if not DO_IT:
