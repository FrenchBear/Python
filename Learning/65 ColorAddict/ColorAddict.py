...
def PlayGame() -> Tuple[List[Card], List[Card], List[Card]]:
    ...
    def Play(player: int) -> Tuple[Optional[Card], bool]:
        ...
        for i in range(Visible[player]):
            c = Decks[player][i]
            if c.Text == top.Text or c.Text == top.Ink or c.Ink == top.Text or c.Ink == top.Ink:
                del Decks[player][i]
                Visible[player] -= 1
...
def PlayOneGame():
    ...
    if len(Deck0) == 0:
        ...
        print(f'Game stuck, Player 1 has {len(Deck0)} card(s), Player 2 has {len(Deck1)} card(s)')
...
def ComputeStats():
    ...
    for i in range(1000):
        ...
        if len(Deck0) == 0:
            ...
            winner = 0
        c[winner] += 1
    print("On 1000 games: 0: Game stuck, 1: Player 1 wins, 2: Player 2 wins, 3: Both players win")
    print(c.most_common())
