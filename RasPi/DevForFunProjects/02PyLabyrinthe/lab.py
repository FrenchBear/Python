...
# 2016-06-01    PV  Adapted for Raspberry Pi
...
# When running in IDLE, ANSI console sequences seem not rendered correctly
simple_output = True
...
if (len(sys.argv)==3):
	try:
		...
		print("lab: rows argument must be an integer")
		sys.exit()
	try:
		columns = int(sys.argv[2])
	except:
		print("lab: columns argument must be an integer")
		sys.exit()
...
def print_labyrinth():
	...
	vertical_wall = '|' if simple_output else '\u2502'
	horizontal_wall = '-' if simple_output else '\u2500'
		...
		for cl in range(columns+1):
			...
			if cl>0:
				if col.walls[0] or rl==rows or rl==0:
					print(horizontal_wall if col.walls[0] else path_dot, end='')
				else:
