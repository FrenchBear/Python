# h2.py - Hilbert curve in graphics mode in Python with pygame
...
# 2016-06-01    PV      Graphical h2 version with polygon fill
...
mod = (height-20)//(side+1)			# size of a square in pixels
iter = 0							# Number of segments drawn to refresh during drawing
fill = False

cx = (width-mod*side)//2            # initial coordinates
cy = height-(height-mod*(side+1))//2
coords = [(cx, cy), (cx, cy-mod)]
cy -= mod
...
coords.append((cx, cy+mod))
pygame.draw.polygon(screen, pygame.Color(255,128,0), coords, 0 if fill else 1)
